...
export default function ReviewMateria({ materia, onBack, showToast, onUpdate }) {
  ...
  async function finishReview(finalSession) {
    ...
    await supabase.from('materias').update({
      ...
      last_review: new Date().toLocaleDateString('es-ES'),
  ...
  async function markDominated() {
    ...
    onUpdate()
    setCelebration({ icon: '⭐', title: '¡Mazo dominado!', sub: `"${materia.name}" marcado como dominado.\n¡Cuando estés listo, declárate Rey!` })
  }
  ...
  if (step === 'config') return (
    <>
      <div className="topbar">
        ...
        <div className="topbar-title">Configurar repaso</div>
        <div style={{ width: 36 }} />
      </div>
      <div className="scroll">
        <div className="card">
          <div className="card-title">¿Cuántas tarjetas?</div>
          {[
            { n: 10, label: '10 tarjetas', sub: 'Repaso rápido ~5 min' },
            { n: 20, label: '20 tarjetas', sub: 'Repaso normal ~10 min' },
            { n: 50, label: '50 tarjetas', sub: 'Repaso intenso ~25 min' },
            { n: 9999, label: 'Todas', sub: 'El mazo completo' },
          ].map(opt => (
        ...
        <div className="card">
          <div className="card-title">Filtrar por dificultad</div>
          {[
            { id: 'all', label: 'Todas' },
            { id: 'hard', label: '🔥 Solo difíciles' },
            { id: 'medium', label: '⚡ Solo medias' },
            { id: 'easy', label: '✅ Solo fáciles' },
          ].map(opt => (
        ...
        <button className="btn btn-primary" onClick={startReview}>▶ Comenzar repaso</button>
      </div>
    </>
  )
  ...
  if (step === 'review' && session) {
    ...
    return <ReviewCard card={card} index={session.index} total={total} pct={pct} onEval={handleEval} onExit={() => { if (confirm('¿Salir del repaso?')) onBack() }} title={materia.name} />
  }
        ...
        <div className="scroll">
          ...
          {showDomination && (
            <div className="notice-box" style={{ marginBottom: 12 }}>
              ...
              <div className="notice-text">Has completado 3 repasos seguidos con más del 80% de aciertos.</div>
              <div className="btn-row">
                <button className="btn btn-gold btn-sm" onClick={markDominated}>Marcar como dominado ⭐</button>
                <button className="btn btn-secondary btn-sm" onClick={onBack}>Seguir repasando</button>
              </div>
            </div>
          )}
      ...
      <div className="scroll">
        ...
        {!revealed ? (
          <>
            ...
            <button className="btn btn-primary" onClick={() => setRevealed(true)}>Revelar respuesta</button>
          </>
        ) : (
          <>
